...
/* #ifdef _MSC_VER
	...
	Vector() { }
	explicit Vector(size_type n) :vector<T>(n) {}
	Vector(size_type n, const T& v) :vector<T>(n, v) {}
	template <class I>
	Vector(I first, I last) : vector<T>(first, last) {}
	Vector(initializer_list<T> list) : vector<T>(list) {}
*/
...
// make min() and max() accessible on systems with antisocial macros:
#undef min
#undef max
